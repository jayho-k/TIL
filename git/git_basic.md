...
충돌시 순서1  git status ==> both modified {파일}이 뜨게 됨 ==> vscode에서 판단을 해줘야 함

충돌시 순거2   pull을 새로하고 다시 올리셈
...
- **git log:** 
  ...
  - git log --oneline --all --graph: 그래프를 그려준다.

  - git reflog: 모든 로그를 보여준다.
...
- git.switch {이름}: 이동하기 // git switch -c new: 브랜치를 만들고 이동할께
...
- git status ==> both modified {파일}이 뜨게 됨 ==> vscode에서 판단을 해줘야 함
...
- 이미 git에 관리가 되어있는 상태이면 ignore을 못함

- touch .gitignore을 생성해준다.

- 메모장에 파일이름을 적는다.

- git status에 파일이름이 없음

- 쓸데없는 파일들 찾기: 구글 ==> gitignore ==> 검색 ==> 메모장복사

  

### 시간여행가기(reset)

 다시 과거로 돌아가서 수정을 해주고 오는 경우
