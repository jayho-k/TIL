...
class JSON2YOLO:
    ...
    def _convert_anno(self,image_info,annotations):
        ...
        for anno in annotations:
            if anno.get('box')==None:
                continue
            bbox = anno['box']
            image_id = anno['image_id']
            class_id = anno['middle classification']
            sub_class_id = anno['class']
            ...
            anno_info = (image_name, class_id, yolo_box, sub_class_id, data_id)
    ...
    def json2yolo(self):
        ...
        for label in self.labels:
            ...
            loaded_json = json.load(open(os.path.join(self.json_dir,label), 'r', encoding='utf-8'))

            image = loaded_json['image']
            annotations = loaded_json['annotations']
            if 'box' not in annotations[0]:
                j += 1
